...
export const ListingCreateBookingModal = ({
    ...
    const [createBooking, { loading }] = useMutation<CreateBookingData, CreateBookingVariables>(CREATE_BOOKING, {
        ...
        onError: () => {
            displayErrorMessage('Sorry! We could not book the listing. Please try again later');
        },
    });
    ...
    const handleCreateBooking = async () => {
        if (!stripe) {
            return displayErrorMessage('Sorry, we were not able to connect with Stripe');
        }
        ...
        if (stripeToken) {
            ...
            displayErrorMessage(
                error && error.message ? error.message : 'Sorry, we could not book the listing, please try again later',
            );
        }
    };
